...
func TestListPositionExtremes(t *testing.T) {
	...
	gtx := layout.Context{
		Ops:         new(op.Ops),
		Constraints: layout.Exact(image.Pt(20, 10)),
	}
	const n = 3
	layout := func(_ layout.Context, idx int) layout.Dimensions {
		if idx < 0 || idx >= n {
...
func TestListScrollToEnd(t *testing.T) {
	...
	l.Layout(gtx, 1, func(gtx layout.Context, idx int) layout.Dimensions {
		return layout.Dimensions{
			Size: image.Pt(10, 10),
		}
	})
...
func TestListPosition(t *testing.T) {
	...
	el := func(gtx layout.Context, idx int) layout.Dimensions {
